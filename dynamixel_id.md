# Konfigurasi ID Servo Dynamixel

Dokumen ini berisi pemetaan ID Dynamixel untuk masing-masing kaki robot (Hexapod / Quadruped) agar mempermudah pemrograman Inverse Kinematics.

### Tabel Pemetaan Kaki

| Posisi Bagian | ID Coxa (Pinggul) | ID Femur (Paha) | ID Tibia (Betis) | Status & Catatan |
| :--- | :---: | :---: | :---: | :--- |
| **Kaki Kiri A** | `4` | `10` | `16` | ✅ **Normal.** Sukses dites IK. *(Catatan: Engsel Tibia sempat mentok mekanis)* |
| **Kaki Kiri B (Pertama)** | `2` | `8` | `14` | ⚠️ **Kendala Kabel.** ID 18 normal jika dicolok langsung (bebas dari bentrok). |
| **Kaki Kiri C** | `1` | `7` | `13` | ✅ **Normal.** Ketiga ID berhasil terdeteksi dari hasil scan (bebas dari bentrok). |
| **Kaki Kanan A** | `6` | `12` | `18` | ✅ **Normal.** Ketiga ID berhasil terdeteksi dari hasil scan. |
| **Kaki Kanan B** | `17`  | `11` | `5` | ⚠️ **Coxa sedang dibersihkan.** Femur & Tibia sukses diubah ke 8 & 14 (Bebas bentrok!). |
| **Kaki Kanan C** | `3` | `9` | `15` | ✅ **Normal.** *(Catatan: Urutan memutar; Coxa 17, Tibia 5 karena injeksi kabel dicolok dari telapak/Tibia).* |

### Tabel Pemetaan Komponen Lain

| Posisi Bagian | ID Servo | Status & Catatan |
| :--- | :---: | :--- |
| **Gripper (Pitch)** | `0` | ✅ **Normal.** Servo gripper lama. |
| **Gripper (Yaw)** | `33` | ✅ **Normal.** Servo baru, berhasil diubah dari ID 1. |

---
*Catatan Tambahan:*
*   *Coxa*: Mengontrol ayunan kaki maju-mundur (X-axis).
*   *Femur & Tibia*: Mengontrol langkah naik-turun (Z-axis).
